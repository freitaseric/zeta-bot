	...
	const embed = createEmbed(interaction.user, {
		...
		fields: [
			...
			},
			{
				...
				value: quote(
					italic(
						`Ainda não possuo uma host, caso conheça uma com preço mais em conta ou seja dono de alguma e queira me ajudar, por favor entre em contato com ${bold(config.developer?.username.toString())} na DM.`,
					),
				),
			},
			{
				name: ':floppy_disk: Banco de Dados',
				value: `⤷\tTipo de Banco: ${inlineCode('MongoDB')}\n⤷\tProvedor Online: ${inlineCode('MongoDB Atlas')}\n⤷\tORM: ${inlineCode('Prisma')} (${italic(packageJson.devDependencies.prisma)})`,
			},
		],
	})
